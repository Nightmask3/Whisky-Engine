...
void PlayerController::Move()
{
	...
	if (INP->IsKeyPressed(KeyCode::Up))
		_gameObject->GetComponent<Transform>()->Translate(0, _moveSpeed*frameTime);
		
	if (INP->IsKeyPressed(KeyCode::Down))
		_gameObject->GetComponent<Transform>()->Translate(0, -_moveSpeed*frameTime);
		
	if (INP->IsKeyPressed(KeyCode::Left))
		_gameObject->GetComponent<Transform>()->Translate(-_moveSpeed*frameTime, 0);
...
{
	vec3 ship_pos = _gameObject->GetComponent<Transform>()->Position();
	vec3 ship_scale = _gameObject->GetComponent<Transform>()->Scale();
